class RenderSpells {
	static $getRenderedSpell (sp, subclassLookup) {
		...
		if (fromClassList.length) {
			...
			renderStack.push(`<tr class="text"><td colspan="6"><span class="bold">职业: </span>${Parser.spMainClassesToFull(current)}</td></tr>`);
			if (legacy.length) renderStack.push(`<tr class="text"><td colspan="6"><section class="text-muted"><span class="bold">职业 (legacy): </span>${Parser.spMainClassesToFull(legacy)}</section></td></tr>`);
		}
		...
		if (fromSubclass.length) {
			...
			renderStack.push(`<tr class="text"><td colspan="6"><span class="bold">子职: </span>${current}</td></tr>`);
			if (legacy.length) {
				renderStack.push(`<tr class="text"><td colspan="6"><section class="text-muted"><span class="bold">子职 (legacy): </span>${legacy}</section></td></tr>`);
			}
		}

		const fromClassListVariant = Renderer.spell.getCombinedClasses(sp, "fromClassListVariant");
		if (fromClassListVariant.length) {
			renderStack.push(`<tr class="text"><td colspan="6"><span class="bold" title="来源: ${Parser.sourceJsonToFull(SRC_UACFV)}">变种职业: </span>${Parser.spMainClassesToFull(fromClassListVariant)}</td></tr>`);
		}

		if (sp.races) {
			sp.races.sort((a, b) => SortUtil.ascSortLower(a.name, b.name) || SortUtil.ascSortLower(a.source, b.source));
			renderStack.push(`<tr class="text"><td colspan="6"><span class="bold">种族: </span>${sp.races.map(r => `${SourceUtil.isNonstandardSource(r.source) ? `<span class="text-muted">` : ``}${renderer.render(`{@race ${r.name}|${r.source}}`)}${SourceUtil.isNonstandardSource(r.source) ? `</span>` : ``}`).join(", ")}</td></tr>`);
		}
